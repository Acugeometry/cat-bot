...
def superspam(session, message):
    ...
    try:
        ...
        if MODE == "studio":
            ...
            session.post_comment_profile(f"{message}", ID_OR_USERNAME)
        elif MODE == "project":
            session.post_comment_project(f"{random.randint(1, 1)} {message}", ID_OR_USERNAME)
        print("oh yeah")
